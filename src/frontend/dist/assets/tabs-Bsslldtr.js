...
import { l as Subscribable, s as shallowEqualObjects, m as hashKey, n as getDefaultState, o as notifyManager, p as useQueryClient, r as reactExports, q as noop, t as shouldThrowError, c as createLucideIcon, j as jsxRuntimeExports, a as cn, f as useActor, v as useQuery, k as ue, w as React, u as useComposedRefs, x as React2, e as createSlot, P as Primitive } from "./index-q08fWabk.js";
...
function useGetMySalon(phone) {
  ...
  return useQuery({
    ...
    retry: 3,
    // retry up to 3 times (cold start)
    retryDelay: (attempt) => Math.min(2e3 * (attempt + 1), 8e3),
    staleTime: 5 * 60 * 1e3
    // 5 min cache — owners rarely change salons
  });
}
function useRegisterSalon(phone) {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (!actor) throw new Error("No actor");
      return actor.registerSalonByPhone(
        phone,
        name,
        address,
        salonPhone,
...
function useGetMyCustomerProfile(phone) {
  ...
  return useQuery({
    ...
    enabled: !!actor && !isFetching && !!phone,
    staleTime: 5 * 60 * 1e3
    // 5 min cache — profile rarely changes
  });
}
...
export {
  TabsContent as A,
  useSaveCustomerProfile as B,
  Card as C,
  useGetAllActiveSalons as D,
  useGetMyAppointments as E,
  useGetSalonServices as F,
  useBookAppointment as G,
  CircleCheckBig as H,
  useGetQueueInfo as I,
  Clock as J,
  useGetMySalon as K,
  LogOut as L,
  useGetOwnerRevenueSummary as M,
  useRegisterSalon as N,
  useGetSalonAppointmentsForDate as O,
  Presence as P,
  useUpdateAppointmentStatus as Q,
  useAddSalonService as R,
  useDeleteSalonService as S,
  Tabs as T,
  useUpdateMySalon as U,
